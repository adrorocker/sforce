...
use Sforce\Email\MassEmailMessage;
use Sforce\Email\SingleEmailMessage;
use Sforce\Process\ProcessRequest;
use Sforce\Process\ProcessSubmitRequest;
use Sforce\Process\ProcessWorkitemRequest;
use Sforce\Proxy\Settings;
use Sforce\HeaderOption\CallOptions;
use Sforce\HeaderOption\AssignmentRuleHeader;
use Sforce\HeaderOption\MruHeader;
use Sforce\HeaderOption\LoginScopeHeader;
use Sforce\HeaderOption\EmailHeader;
use Sforce\HeaderOption\UserTerritoryDeleteHeader;
use Sforce\HeaderOption\AllowFieldTruncationHeader;
use Sforce\HeaderOption\LocaleOptions;
use Sforce\HeaderOption\PackageVersionHeader;
use Sforce\HeaderOption\PackageVersion;
use Sforce\FieldType\Definitions;
...
class BaseClient
{
     ...
     */
    public function createConnection($wsdl, Settings $proxy = null, $soapOptions = [])
    {
    ...
    public function login($username, $password)
    {
        ...
        if ($this->callOptions != null) {
            $this->sforce->__setSoapHeaders(array(
                $this->callOptions
            ));
        }
        if ($this->loginScopeHeader != null) {
            $this->sforce->__setSoapHeaders(array(
                $this->loginScopeHeader
            ));
        }
